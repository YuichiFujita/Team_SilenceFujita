...
#define VIEW_FAR		(16000.0f)				// モデルが見えるZ軸の最大値
...
void MoveFollowCamera(void)
{
	...
	case PLAYCAMESTATE_NORMAL:			// 通常のカメラ状態
		...
		{ // 一人称がONの場合
			// 目標の視点の位置を更新
			g_camera.posV.x = pPlayer->pos.x + sinf(pPlayer->rot.y) * 18.5f;				// 目標注視点から距離分離れた位置
			g_camera.posV.y = pPlayer->pos.y + 80.0f;		// 固定の高さ
			g_camera.posV.z = pPlayer->pos.z + cosf(pPlayer->rot.y) * 18.5f;				// 目標注視点から距離分離れた位置
	...
	case PLAYCAMESTATE_BACK:			//後ろを見るカメラ状態
		...
		{ // 一人称がONの場合
			// 目標の視点の位置を更新
			g_camera.posV.x = pPlayer->pos.x - sinf(pPlayer->rot.y) * 35.0f;				// 目標注視点から距離分離れた位置
			g_camera.posV.y = pPlayer->pos.y + 80.0f;		// 固定の高さ
			g_camera.posV.z = pPlayer->pos.z - cosf(pPlayer->rot.y) * 35.0f;				// 目標注視点から距離分離れた位置
